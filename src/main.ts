...
import { fuelDebitAndRegimeAdapter } from './injection/infra/adapters/fuel-debit.js';
...
const controllers = (config: Config): Controllers => {
  const fuelDebitAndRegime = fuelDebitAndRegimeAdapter(
    defaultHttpClient(),
    config.fuelDebitUrl
  );
  ...
  return {
    ...
    getInjectionDebit: getInjectionDebitController(
      getInjectionDebitUseCase({
        rules: createRules(fuelDebitAndRegime),
        rulesRepository: inMemoryRulesRepository(),
        fuelDebitAndRegimeAdapter: fuelDebitAndRegime,
      })
    ),
